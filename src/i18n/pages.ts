...
const en = {
    ai: {
        ...
        capabilities: {
            ...
            customTools: { title: "Custom tools for your stack", tagline: "An agent that acts, not just chats", description: "A model can only talk. We write the **typed tools** that connect it to your actual systems — your CRM, your database, your internal APIs — so the agent does real work: reads the record, sends the email, updates the order. Every tool is scoped and idempotent, so it can act without breaking things.", bullets: ["Tools built for your CRM, ERP, database and internal APIs", "Scoped permissions per tool — it only does what you allow", "Idempotent actions, so a retry never double-charges", "The agentic loop that orchestrates them, plus recovery"] },
...
const es: typeof en = {
    ai: {
        ...
        capabilities: {
            ...
            customTools: { title: "Herramientas a medida para tu stack", tagline: "Un agente que actúa, no solo conversa", description: "Un modelo solo sabe hablar. Nosotros escribimos las **herramientas tipadas** que lo conectan a tus sistemas reales — tu CRM, tu base de datos, tus APIs internas — para que el agente haga trabajo de verdad: lee el registro, manda el email, actualiza el pedido. Cada herramienta está acotada y es idempotente, así puede actuar sin romper nada.", bullets: ["Herramientas hechas para tu CRM, ERP, base de datos y APIs internas", "Permisos acotados por herramienta — solo hace lo que le permitís", "Acciones idempotentes, así un reintento nunca cobra dos veces", "El loop agéntico que las orquesta, más la recuperación"] },
...
const pt: typeof en = {
    ai: {
        ...
        capabilities: {
            ...
            customTools: { title: "Ferramentas sob medida para o seu stack", tagline: "Um agente que age, não só conversa", description: "Um modelo só sabe falar. Nós escrevemos as **ferramentas tipadas** que o conectam aos seus sistemas de verdade — seu CRM, seu banco de dados, suas APIs internas — para o agente fazer trabalho real: lê o registro, manda o email, atualiza o pedido. Cada ferramenta é limitada e idempotente, então ela pode agir sem quebrar nada.", bullets: ["Ferramentas feitas para o seu CRM, ERP, banco de dados e APIs internas", "Permissões limitadas por ferramenta — só faz o que você permite", "Ações idempotentes, então um retry nunca cobra duas vezes", "O loop agêntico que as orquestra, além da recuperação"] },
