...
const TechnologiesScrollListInner = ({ techs, direction }: any) => {
  ...
  useEffect(() => {
    if (direction === -1) {
      scrollContainerRef.current!.scrollLeft =
        (scrollContainerRef.current?.scrollWidth ?? 0) * 2;
    }
  }, []);
  ...
  return (
    <div
      ...
      className="flex flex-row gap-4 overflow-x-hidden pb-4 px-4 no-scrollbar relative w-full max-w-full min-w-full"
    >
